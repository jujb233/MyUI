...
SolidJS 用法：子组件均作为属性挂载在默认导出的 `MyCard` 组件上：

```tsx
import MyCard from '../Components/MyCard';

<MyCard backgroundImage="/demo.png">
  <MyCard.Tags tags={["SolidJS", "TypeScript"]} />
...
SolidJS 说明：插槽内容和子组件均通过 JSX 语法传递，事件类型为原生 `MouseEvent`，插槽类型为 `JSX.Element`。
